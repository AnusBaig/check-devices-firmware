...
export function newProduct ({commit},payload) {
  const newProduct={
    ...
    currentFirmware:{id:-1,version:"Not specified"},
    devices:[],
    name:payload,
  ...
  updatedFirmware.push({productId:newProduct.id,firmwares:[]});
  localStorage.setItem("productsFirmwares",JSON.stringify(updatedFirmware));

  commit("common/addNewProduct",{id:newProduct.id,name:payload},{root:true});
  commit("common/selectProduct",newProduct.id,{root:true});

  commit("updateProductInfo",{name:newProduct.name,OTAUrl:newProduct.OTAUrl,id:newProduct.id});
