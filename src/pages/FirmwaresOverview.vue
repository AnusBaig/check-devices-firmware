<template>
  <q-page padding>
    <div v-if="getSelectedProduct">
      <div @click.stop="onCardClick" class="">
        <q-card flat square class="text-center">
          <q-card-section class="row d-inline items-center">
            <q-avatar
              size="42px"
              font-size="24px"
              color="green"
              ...
              icon="fact_check"
            />
            <span class="q-ml-md text-h6 text-positive">Active Firmwares</span>
          </q-card-section>
          <q-card-section class="text-left q-mt-md">
            <div class="shadow-card">
              <div class="row q-gutter-md q-pa-lg shadow-card-fix">
                <q-card
      ...
      <div>
        <q-card flat square class="text-center q-mt-xl ">
          <q-card-section class="row items-center d-inline">
            <q-avatar
              size="42px"
              font-size="24px"
              color="red"
              ...
              icon="warning"
            />
            <span class="q-ml-md text-h6 text-red-6">Inactive Firmwares</span>
          </q-card-section>
      ...
      <div class="dialogDiv">
        ...
        >
          <q-card style="min-width:600px" class="q-pa-xl">
            <q-bar class="bg-white">
              <q-space />
              <div class="row items-center q-my-lg">
                <q-avatar icon="devices" color="teal" text-color="white" />
                <span
                  class="q-ml-sm text-body1 text-h4 font-helvetica text-teal-9 font-weight-bold"
                  >New Firmware</span
                >
              </div>
              <q-space />
              <q-btn dense flat icon="close" color="text-purple" v-close-popup>
                <q-tooltip content-class="bg-white text-purple"
                  >Close</q-tooltip
                >
              </q-btn>
            </q-bar>
            ...
            <q-card-section class="row items-center justify-center q-my-lg">
              <q-btn
                unelevated
                class="q-px-md"
                size=".8rem"
...
export default {
  ...
  created() {
    if (this.getSelectedProduct) {
      var firmwares = JSON.parse(
        localStorage.getItem("productsFirmwares")
      ).find(v => v.productId == this.getSelectedProduct).firmwares;
