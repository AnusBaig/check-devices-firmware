      ...
      <q-card-section>
        ...
        <div class="row items-center q-mb-md q-gutter-lg">
          ...
          <span class="text-body1">
            {{ firmwareInfo(id).deviceIds.length }}</span
          >
        </div>
        <div class="row items-center q-mb-md q-gutter-lg">
          <span class="text-body1"> Signed:</span>
          <q-icon
            v-if="firmwareInfo(id).signed"
            name="check_circle"
            color="green"
            size="20px"
          />
      ...
      <q-card-section>
        ...
        <div class="editorHolder">
          <ckeditor
            v-if="editMode"
            :editor="editor"
            v-model="editedReleaseData"
            :config="editorConfig"
          ></ckeditor>
          <div
            class="releaseNotesDisplay q-pa-md"
            v-html="editedReleaseData"
            v-else
          ></div>
        </div>
      </q-card-section>
  ...
  methods: {
    ...
    saveData() {
      this.saveReleaseNotes({
        releaseNotes: this.editedReleaseData,
        firmwareId: this.id
      });
      this.editMode = false;
    }
